...
public class IncreaseInventoryDialog extends Dialog {
    ...
    public IncreaseInventoryDialog(
            InventoryService inventoryService,
            TranslationService translations,
            ProductDto product,
            Consumer<Void> onSuccessReload // pass v -> reload()
    ) {
        ...
        setHeaderTitle(translations.t("inventory.increaseTitle", product.getName()));
        ...
        // NumberField is Double; convert to BigDecimal
        ...
        Span hint = new Span(translations.t("inventory.increaseHint"));
        ...
        Button save = new Button(translations.t("inventory.increaseButton"));
        save.addThemeVariants(ButtonVariant.LUMO_PRIMARY);
        save.addClickShortcut(Key.ENTER);
        save.addClickListener(e -> {
            if (saveAndApplyIncrease(product)) {
                close();
    ...
    private boolean saveAndApplyIncrease(ProductDto product) {
        try {
            binder.writeBean(bean);

            inventoryService.increaseStock(bean.getProductId(), bean.getQuantity(), bean.getNote());

            NotificationSupport.showSuccess(
                    translations.t("inventory.increasedNotification", product.getName(), bean.getQuantity().toPlainString())
            );
            return true;
